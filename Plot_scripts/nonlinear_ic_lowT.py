...
t_arr = np.array([0,nlinf['nlast']-210,nlinf['nlast']-10])
...
gap_s1 = [350,2350,4350]
gap_s2_0 = 650

gap_l1 = [1250,3250,5250]
gap_l2_0 = 1750

gap_s = gap_s2_0-gap_s1[0]
gap_l = gap_l2_0-gap_l1[0]

for x in gap_s1:
    ax[2,0].axvspan(x, x+gap_s, alpha=0.3, color='gray')
    ax[2,1].axvspan(x, x+gap_s, alpha=0.3, color='gray')
    ax[2,2].axvspan(x, x+gap_s, alpha=0.3, color='gray')
    
    ax[0,1].axvspan(x, x+gap_s, alpha=0.3, color='gray')
    ax[0,2].axvspan(x, x+gap_s, alpha=0.3, color='gray')

for x in gap_l1:
    ax[2,0].axvspan(x, x+gap_l, alpha=0.3, color='gray')
    ax[2,1].axvspan(x, x+gap_l, alpha=0.3, color='gray')
    ax[2,2].axvspan(x, x+gap_l, alpha=0.3, color='gray')

    ax[0,1].axvspan(x, x+gap_l, alpha=0.3, color='gray')
    ax[0,2].axvspan(x, x+gap_l, alpha=0.3, color='gray')
...
T1_arr = np.zeros_like(x1) + T1
T2_arr = np.zeros_like(x1) + T2
T3_arr = np.zeros_like(x1) + T3

rho1_arr = np.zeros_like(x1) + rho1

ax[2,0].plot(x1*ul,T1_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)
ax[2,0].plot(x1*ul,T2_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)

ax[2,1].plot(x1*ul,T3_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)
ax[2,2].plot(x1*ul,T3_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)

#ax[0,2].set_ylim(0.061999,0.062001)
#ax[0,2].set_xlim(1450,1600)
#ax[0,1].set_ylim(0.062,0.062001-5e-7)
#ax[0,1].set_xlim(1450,1600)

#ax[0,1].plot(x1*ul,rho1_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)
#ax[0,2].plot(x1*ul,rho1_arr,linestyle='dashed',color='k',linewidth=plotline_width-2)

plt.savefig(wdir_script+'/nonlinear/nonlinear_ic_lowT.png')
