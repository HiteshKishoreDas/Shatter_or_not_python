        ...
        **Note**:
        ...
        for i in range(il+1):
            ...
            for j in range(nbox): # loop on all boxes of a given level
                AMRLevel[i]['box'].append({'x0':0.,'x1':0.,'ib':six.long(0),'ie':six.long(0),\
                                           'y0':0.,'y1':0.,'jb':six.long(0),'je':six.long(0),\
                                           'z0':0.,'z1':0.,'kb':six.long(0),'ke':six.long(0)})
                # Box indexes
...
class Tools(object):
    ...
    def sph2cyl(self,D,Dx,rphi=None,theta0=None):
        """ This method transforms spherical data into cylindrical applying \
        interpolation. Works for stretched grid as well, transforms poloidal\
        (R-Theta) data by default. Fix theta and set rphi=True to get (R-Phi)\
        transformation.
        **Inputs:**
